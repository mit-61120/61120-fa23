...
class Term {
 public:
  Term(int32_t value, TermPrime *rest) : value_(value), rest_(rest) {}
...
class TermPrimeMul : public TermPrime {
 public:
  TermPrimeMul(int32_t value, TermPrime *rest) : value_(value), rest_(rest) {}
...
class TermPrimeDiv : public TermPrime {
 public:
  TermPrimeDiv(int32_t value, TermPrime *rest) : value_(value), rest_(rest) {}
  ...
  switch (token->getType()) {
    ...
    case MITScript::DIV: {
      ...
      if (nextToken->getType() == MITScript::INT) {
        ...
        switch (token->getType()) {
          case MITScript::MUL:
            return new TermPrimeMul(value, rest);
          case MITScript::DIV:
            return new TermPrimeDiv(value, rest);
          default: {
            assert(0);
          }
        }
