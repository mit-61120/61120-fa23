...
run_mem_tests(){
    ...
    run_and_parse_mem() {
        ...
        rm -f tmp.out
        USAGE=$(timeout $TIMEOUT time -v $INTERPRETER -mem $2 -s $PREFIX.mit < $PREFIX.input 2>&1 > tmp.out | grep Max | awk '{print $6}')
    ...
    #run_and_parse_mem kmediods 30
    #run_and_parse_mem life 6
