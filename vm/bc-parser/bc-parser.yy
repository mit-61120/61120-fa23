...
Function
{
	auto list = new std::vector<Function *>();
...
| T_int
{
	$$ = new Constant::Integer{safe_cast($1)};
}

ConstantListStar:
%empty { $$ = new std::vector<Constant *>(); }
| ConstantListPlus
{
	$$ = $1;
}
...
Constant
{
	auto list = new std::vector<Constant *>();
...
int yyerror(BCLTYPE * yylloc, void* p, Function*& out, const char*  msg){
  ...
  std::cout<<"Error in line "<<yylloc->last_line<<", col "<<yylloc->last_column<<": "<<msg <<std::endl;
  return 0;
}
