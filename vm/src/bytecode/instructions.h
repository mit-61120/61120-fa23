...
enum class Operation {
  ...
  // Operand 0:   index of name of global variable in enclosing function's names
  // list Operand 1:   value to store Stack:       S :: operand 1 ==> S
  StoreGlobal,

  // Description: push a reference to a local variable or free variable
  // reference onto the operand stack Mnemonic:    push_ref i Operand 0:   index
  // of local variable reference Stack:       S ==>  S :: address_of(var)
  //                  where var = i < f.local_reference_vars.size() ?
  //                  f.local_reference_vars[i]
  //                                                           :  f.free_vars[i
  //                                                           -
  //                                                           f.local_reference_vars.size()]
  PushReference,
  ...
  // Operand 0:   index of the field's name within the enclosing function's
  // names list Operand 1:   record from which to load Stack:       S :: operand
  // 1 => S :: record_value_of(operand 1, f.names[i])
  FieldLoad,

  // Description: store value into field of record
  // Mnemonic:    field_store i
  // Operand 0:   index of the field's name within the enclosing function's
  // names list Operand 1:   the value to store Operand 2:   the record to store
  // into Stack:       S :: operand 2 :: operand 1 => S
  FieldStore,

  // Description: load value from index of record
  // Mnemonic:    index_load
  // Operand 0:   N/A
  // Operand 1:   the index to read from (can be arbitrary value. indexing
  // adheres to semantics of Assignment #2) Operand 2:   the record to read from
  // Stack:       S :: operand 2 :: operand 1 => S :: record_value_of(operand 2,
  // operand 1)
  IndexLoad,
  ...
  // Operand 2:   the index to store to (can be arbitrary value. indexing
  // adheres to semantics of Assignment #2) Operand 3:   the record to store
  // into Stack:       S :: operand 3 :: operand 2 :: operand 1 => S
  IndexStore,
  ...
  // Operand i  (0 < i <= m) : free variable argument (m - i). Argument (m - i)
  // maps to position (m - i) Operand n=m+1 : function Stack:       S :: operand
  // n :: ... :: operand 3 :: operand 2 :: operand 1 => S :: closure
  AllocClosure,
  ...
  // Stack:       S::operand n :: .. :: operand 3 :: operand 2 :: operand 1 => S
  // :: return value
  Call,

  // Description: ends the execution of the enclosing function and returns the
  // top of the stack Mnemonic:    return Operand 0:   N/A Operand 1:   value to
  // return Stack::      S :: operand 1 => S
  Return,

  // Description: implements addition (as given in the semantics of Assignment
  // #2) Mnemonic:    add Operand 0:   N/A Operand 1:   right value Operand 2:
  // left value Result:      value of + operation as specified by the semantics
  // of Assignment #2 Stack:       S:: operand 2 :: operand 1 => S :: (operand
  // 2) + (operand 1)
  Add,
  ...
  // Description: computes an equality between two values (semantics according
  // to Assignment #2) Mnemonic:    eq Operand 0:   N/A Operand 1:   right value
  // Operand 2:   left value
  // Stack:       S :: operand 2 :: operand 1 => S:: eq(operand 2, operand 1)
  Eq,

  // Description: computes a boolean operation on two boolean operands
  // (semantics according to Assignment #2) Mnemonic:    and/or Operand 0:   N/A
  // Operand 1:   right value
  // Operand 2:   left value
  // Stack:       S :: operand 2 :: operand 1 => S:: op(operand 2, operand 1)
  And,
  ...
  // Description: transfers execution of the function to a new instruction
  // offset within the current function Mnemonic:    goto i Example: goto 0
  // jumps to the current instruction. goto 1 jumps to the following
  // instruction. goto -1 jumps to the preceding instruction Operand 0: offset
  // relative to the current instruction offset to jump to. Stack:       S => S
  Goto,

  // Description: transfers execution of the function to a new instruction
  // offset within the current function if the operand evaluates to true.
  // Mnemonic:    if i
  // If operand evaluates to false, then execution continues at the next
  // instruction Operand 0:   offset relative to the current instruction offset
  // to jump to. Operand 1:   value Stack:       S :: operand 1 => S
  If,

  // Description: duplicates the element at the top of the stack.
  // Mnemonic:    dup
  // If this element is a reference to a record, function, or local variable,
  // the operation only duplicates the reference. Operand 0:   N/A Operand 1:
  // value Stack:       S :: operand 1 => S :: operand 1 :: operand 1
  Dup,
